...
public class GetByIdOrderCourierEndpoint : Endpoint<GetOrderByIdCourierRequest, Results<Ok<IEnumerable<OrderResponse>>, UnauthorizedHttpResult, NotFound, ProblemDetails>>
{
  private readonly AppDbContext _dbContext;

  public GetByIdOrderCourierEndpoint(AppDbContext dbContext)
  {
  ...
  {
    Get("/orders/assigned-to-courier/{id}");
    Summary(s =>
    {
      s.Summary = "Get order by id, assigned to the authenticated courier";
  ...
  public override async Task<Results<Ok<IEnumerable<OrderResponse>>, UnauthorizedHttpResult, NotFound, ProblemDetails>> ExecuteAsync(GetOrderByIdCourierRequest req, CancellationToken ct)
  {
    ...
    // Obtener el pedido por id, que tienen ítems asignados al mensajero autenticado
