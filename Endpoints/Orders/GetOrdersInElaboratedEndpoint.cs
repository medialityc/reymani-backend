...
public class GetOrdersInElaboratedEndpoint : Endpoint<GetOrdersInElaboratedRequest, Results<Ok<IEnumerable<OrderResponse>>, UnauthorizedHttpResult, NotFound, ProblemDetails>>
{
  private readonly AppDbContext _dbContext;

  public GetOrdersInElaboratedEndpoint(AppDbContext dbContext)
  {
  ...
  {
    Get("/orders/elaborate");
    Summary(s =>
    {
      s.Summary = "Get all orders and products in elaboration";
      s.Description = "Retrieves a list of all orders with items in elaboration that belong to the authenticated business admin.";
    });
    Roles("BusinessAdmin");
  }

  public override async Task<Results<Ok<IEnumerable<OrderResponse>>, UnauthorizedHttpResult, NotFound, ProblemDetails>> ExecuteAsync(GetOrdersInElaboratedRequest req, CancellationToken ct)
  {
    ...
    foreach (var r in response)
    {
      ...
      r.Items.Clear();
      r.Items = itemsResponse.ToList(); // Asignar la lista filtrada directamente
    }
