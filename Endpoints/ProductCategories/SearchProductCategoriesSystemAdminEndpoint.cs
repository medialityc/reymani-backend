...
{
  public class SearchProductCategoriesSystemAdminEndpoint(AppDbContext dbContext, IBlobService blobService)
      : Endpoint<SearchProductCategoriesSystemAdminRequest, Results<Ok<PaginatedResponse<ProductCategoryResponse>>, ProblemDetails>>
  {
    public override void Configure()
    {
      Get("/product-categories/search/system-admin");
      Roles("SystemAdmin");
      Summary(s =>
      {
        s.Summary = "Search product categories";
        s.Description = "Searches for product categories based on the specified criteria. Only available for System Admins.";
      });
    }

    public override async Task<Results<Ok<PaginatedResponse<ProductCategoryResponse>>, ProblemDetails>>
        ExecuteAsync(SearchProductCategoriesSystemAdminRequest req, CancellationToken ct)
    {
      ...
      if (req.IsActive is not null)
        query = query.Where(pc => pc.IsActive == req.IsActive);
